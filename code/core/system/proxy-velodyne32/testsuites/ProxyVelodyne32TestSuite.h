/**
 * proxy-velodyne32 - Interface to Velodyne HDL-32E.
...
#ifndef PROXY_PROXYVELODYNE32_TESTSUITE_H
#define PROXY_PROXYVELODYNE32_TESTSUITE_H
...
#include "../include/velodyne32Decoder.h"
...
class packetToByte : public odcore::io::conference::ContainerListener {
   public:
    packetToByte(std::shared_ptr< odcore::wrapper::SharedMemory > m1, std::shared_ptr< odcore::wrapper::SharedMemory > m2)
        : m_mcc(m2)
        , m_velodyne32decoder(m1, m_mcc, "../HDL-32E.xml", false, 0, 0, 0, 1, 1) //The calibration file HDL-32E.xml is automatically copied to the parent folder of the test suite binary
    {}
    ...
    virtual void nextContainer(odcore::data::Container &c) {
        if (c.getDataType() == odcore::data::pcap::Packet::ID()) {
            ...
            if (packetHeader.getIncl_len() == 1248) {
                ...
                m_velodyne32decoder.nextString(payload);     //Let the Velodyne32 decoder decode the 1206-byte payload
            }
        }
    }
   ...
   private:
    ...
    opendlv::core::system::proxy::Velodyne32Decoder m_velodyne32decoder;
};

class ProxyVelodyne32Test : public CxxTest::TestSuite {
   public:
    ProxyVelodyne32Test()
    ...
    void readCsvFile() {                                              //Read a .csv file that is compressed into a .zip file
        fstream fin("../sampleVeloViewFrame1_velodyne32.zip", ios::binary | ios::in); //Read xyz and intensity values of points in Frame 1 exported from VeloView. These values exist in a zip file in the parent folder
        ...
        while (getline(decompressedData, value)) { //Read the csv file line by line and save x, y, z, intensity values of all points in Frame 1 to respective vectors for comparison with our Velodyne32 decoder
    ...
    void testVelodyneDecodingFromFile() {
        ...
        fstream lidarStream("../sampleShort_velodyne32.pcap", ios::binary | ios::in); //A sample .pcap file containing several Velodyne frames in the parent folder
        ...
        TS_ASSERT(static_cast<float>(compare)/static_cast<float>(m_xDataV.size())>0.99f);  //At least 99% of all the points of Frame 1 should be matched between Velodyne32Decoder and VeloView for the sample pcap file.
    }

   private:
    const uint32_t m_BUFFER_SIZE = 4000;
    const std::string m_NAME = "testVelodyne32SM"; //The name for the shared memory m_velodyneSharedMemory
    const std::string m_NAME2 = "sharedSegment32";   //The name for the shared memory segment
    const uint32_t m_SIZE = 1120000;
    //The total size of the shared memory: MAX_POINT_SIZE * NUMBER_OF_COMPONENTS_PER_POINT * sizeof(float), where MAX_POINT_SIZE is the maximum number of points per frame (This upper bound should be set as low as possible, as it affects the shared memory size and thus the frame updating speed), NUMBER_OF_COMPONENTS_PER_POIN=4 (x, y, z, intensity) Recommended values: MAX_POINT_SIZE=70000->ProxyVelodyne32.sharedMemory.size = 1120000
...
#endif /*PROXY_PROXYVELODYNE32_TESTSUITE_H*/
