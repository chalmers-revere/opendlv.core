...
class ProxyVelodyne16Test : public CxxTest::TestSuite {
   public:
    ...
    void readCsvFile() {                                              //Read a .csv file that is compressed into a .zip file
        ...
        while (getline(decompressedData, value)) { //Read the csv file line by line and save x, y, z, intensity values of all points in Frame 1 to respective vectors for comparison with our Velodyne16 decoder
            ...
            m_azimuthV.push_back(stof(cell));
            getline(lineStream, cell, ',');
            m_distanceV.push_back(stof(cell));
            getline(lineStream, cell, '\n');
    ...
    void testVelodyneDecodingFromFile() {
        ...
        if (m_segment->isValid()) {
            ...
            for (uint32_t i = 0; i < m_distanceV.size(); i++) {
                //90000 assumes at most 30000 points per frame and 3 values per point: azimuth, distance, intensity
                for (uint32_t j = spcCounter; j < 90000; j += 3) {
                    if ((abs(velodyneRawData[j] - m_azimuthV[i] / 100.0f) < 0.15f) && ((abs(velodyneRawData[j + 1] - m_distanceV[i])) < 0.1f) && ((abs(velodyneRawData[j + 2] - m_intensityV[i])) < 0.1f)) {
                        spcCounter = j;
                        compare++;
                        break;
                    }
   ...
   private:
    ...
    vector< float > m_azimuthV;
    vector< float > m_distanceV;
